...
##############################
Outputs of *fMRIPost-resample*
##############################

*fMRIPost-resample* outputs conform to the :abbr:`BIDS (brain imaging data structure)`
Derivatives specification (see `BIDS Derivatives`_, along with the
upcoming `BEP 011`_ and `BEP 012`_).
*fMRIPost-resample* generates three broad classes of outcomes:
...
1.  **Visual QA (quality assessment) reports**:
    ...
    of processing and ensures the transparency of *fMRIPost-resample* operation.
...
Assuming fMRIPost-resample is invoked with::

    fmripost_resample <input_dir>/ <output_dir>/ participant [OPTIONS]
...
``dataset_description.json`` is a metadata file in which fMRIPost-resample
...
*fMRIPost-resample* outputs summary reports,
written to ``<output dir>/fmripost_resample/sub-<label>.html``.
These reports provide a quick way to make visual inspection of the results easy.


**********************************
Derivatives of *fMRIPost-resample*
**********************************
...
For each :abbr:`BOLD (blood-oxygen level dependent)` run processed with *fMRIPost-resample*,
...
*fMRIPost-resample* outputs a set of confounds that can be used to denoise the data.
...
The confounds generated by *fMRIPost-resample* are ICA component time series
...
See implementation on :mod:`~fmripost_resample.workflows.bold.confounds.init_bold_confs_wf`.
