...
def main():
    ...
    from fmripost_resample.cli.parser import parse_args
    from fmripost_resample.cli.workflow import build_workflow
    from fmripost_resample.utils.bids import write_bidsignore, write_derivative_description
    ...
    if 'pdb' in config.execution.debug:
        from fmripost_resample.utils.debug import setup_exceptionhook
    ...
    if not config.execution.notrack and not config.execution.debug:
        ...
        from fmripost_resample.utils.telemetry import sentry_setup, setup_migas
    ...
    fmripost_resample_wf = retval.get('workflow', None)

    # CRITICAL Load the config from the file. This is necessary because the ``build_workflow``
    # function executed constrained in a process may change the config (and thus the global
    # state of fMRIPost-resample).
    config.load(config_file)

    if config.execution.reports_only:
        sys.exit(int(EXITCODE > 0))

    if fmripost_resample_wf and config.execution.write_graph:
        fmripost_resample_wf.write_graph(graph2use='colored', format='svg', simple_form=True)

    EXITCODE = EXITCODE or (fmripost_resample_wf is None) * EX_SOFTWARE
    if EXITCODE != 0:
        sys.exit(EXITCODE)

    # Generate boilerplate
    with Manager() as mgr:
        from fmripost_resample.cli.workflow import build_boilerplate

        p = Process(target=build_boilerplate, args=(str(config_file), fmripost_resample_wf))
        p.start()
        p.join()
    ...
    if sentry_sdk is not None:
        ...
        sentry_sdk.add_breadcrumb(message='fMRIPost-resample started', level='info')
        sentry_sdk.capture_message('fMRIPost-resample started', level='info')

    config.loggers.workflow.log(
        15,
        '\n'.join(
            ['fMRIPost-resample config:'] + [f'\t\t{s}' for s in config.dumps().splitlines()]
        ),
    )
    config.loggers.workflow.log(25, 'fMRIPost-resample started!')
    errno = 1  # Default is error exit unless otherwise set
    try:
        fmripost_resample_wf.run(**config.nipype.get_plugin())
    except Exception as e:
        if not config.execution.notrack:
            from fmripost_resample.utils.telemetry import process_crashfile
        ...
        config.loggers.workflow.critical('fMRIPost-resample failed: %s', e)
        raise

    else:
        config.loggers.workflow.log(25, 'fMRIPost-resample finished successfully!')
        if sentry_sdk is not None:
            success_message = 'fMRIPost-resample finished without errors'
        ...
        if boiler_file.exists():
            ...
            config.loggers.workflow.log(
                25,
                'Works derived from this fMRIPost-resample execution should include the '
        ...
        from fmripost_resample.reports.core import generate_reports
...
def migas_exit() -> None:
    ...
    from fmripost_resample.utils.telemetry import send_breadcrumb
...
if __name__ == '__main__':
    raise RuntimeError(
        'fmripost_resample/cli/run.py should not be run directly;\n'
        'Please `pip install` fmripost_resample and use the `fmripost_resample` command'
    )
